...
async function composePoster(imageUrl: string): Promise<Buffer> {
  ...
  try {
    ...
    console.warn(`Frame image not found: ${template.framePath}, skipping frame overlay`);
  }
...
async function uploadToR2(buffer: Buffer, filename: string): Promise<string> {
  ...
  // R2の公開URLを返す
  const publicUrl = `https://pub-4215f2149d4e4f369c2bde9f2769dfd4.r2.dev/generated/${filename}`;
  return publicUrl;
}
...
router.post('/', async (req, res) => {
  try {
    ...
    console.log(`[compose-poster] Poster created: ${posterUrl}`);
