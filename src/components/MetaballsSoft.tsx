  ...
  ({ animated = true, onInteract, profile, morphing = false, morphDelayMs = 0 }, ref) => {
    ...
    useEffect(() => {
      ...
      if (!targetProfileRef.current) {
        return; // 初期状態 (INITIAL_WHITE_PROFILE) を維持
      }
      ...
      const interpolated: UserUrulaProfile = {
        ...
        mat_weights: {
          ...
          glassribpattern: defaultProfile.mat_weights.glassribpattern * (1 - t) + target.mat_weights.glassribpattern * t,
          ...
          ripstop: defaultProfile.mat_weights.ripstop * (1 - t) + target.mat_weights.ripstop * t,
          satin_silk: defaultProfile.mat_weights.satin_silk * (1 - t) + target.mat_weights.satin_silk * t,
          suede: defaultProfile.mat_weights.suede * (1 - t) + target.mat_weights.suede * t,
          velvet: defaultProfile.mat_weights.velvet * (1 - t) + target.mat_weights.velvet * t,
          wool: defaultProfile.mat_weights.wool * (1 - t) + target.mat_weights.wool * t,
        },
        ...
        >
          ...
          <group rotation={[0, rotation, 0]}>
            <MarchingCubes
              ...
              enableColors={false}
            >
              {textures && currentProfile && (
                // モーフィング中または進化済みの場合はBlendedMaterialを使用
                morphing || currentProfile.history.generations > 0
              ) ? (
                <BlendedMaterial
                  textures={textures}
